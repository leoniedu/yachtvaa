...
##packages <- c("~/github/yachtvaa/", "~/github/rsimcosta/", "~/github/rtreinus/", "~/github/rsiscorar/"); lapply(packages, devtools::document); lapply(packages, devtools::install)
...
records_sf <- records_to_sf(records) |>
  ## fix tz for athlete 50 (FIX: where can we get tz??)|>
  mutate(timestamp=if_else(id_athlete==50, timestamp+(60*60*3), timestamp))
...
# Create cache system for rsiscorar data
area <- "baiatos"  # Using the area from the original call
date_str <- gsub("-", "", params$date)  # Format date as YYYYMMDD
hours_str <- paste(0:23, collapse = "-")  # Hours as string

library(rsiscorar)
grib <- rsiscorar::get_grib(date = params$date, area = area, resolution = 0.001)
...
paddlers |>
  ...
  mutate(
    ...
    first_fix=format.Date(lubridate::with_tz(first_fix, "America/Recife"), "%H:%M")
  ) |>
  select(id_athlete, fullname_athlete, first_fix, duration_fmt, track_distance_km, speed_kmh) |>
  arrange(desc(speed_kmh)) |>
  gt::gt() |>
  gt::cols_label(
id_athlete = "Treinus ID", 
...
```{r fastest-x, include = FALSE}
fastx <- fastest_straight_distance(
  records_sf_bbox,
  athlete_col = "id_athlete",
  time_col    = "timestamp",
  distance_m  = params$length_m
)
...
if (nrow(fastx) > 0) {
  ...
  fastx <- fastx[start_in_bbox & end_in_bbox, ]
}
...
```{r fastest-map, fig.height = 8.6, fig.width = 13.8, fig.cap = "Trechos mais rápidos de X m em linha reta. Setas indicam a direção do percurso."}
arrow_df <- fastx |>
  as_tibble()
...
```{r current-field, fig.height = 8.6, fig.width = 8.8, fig.cap = "Campo de correntes oceânicas SISCORAR. Setas indicam direção e intensidade."}
...
ggplot() +
  ...
  labs(
    title    = paste0("Trechos mais rápidos (", params$length_m, "m) e campo de correntes"),
    subtitle = paste0(
...
matched <- match_buoy_to_segments(fastx, buoy_for_match)
```
...
### Classificação: `r params$length_m`m mais rápidos
...
cols_label <- list(
  ...
  inicio="Início",
  tempo = "Tempo",
  avg_speed_kmh = "Velocidade (km/h)", vento = "Comp. Vento",
  corrente_siscorar = "Comp. Corrente (SISCORAR)"
)
...
if (has_buoy_current &&
  ...
  cols_label$corrente_boia <- "Comp. Corrente (Boia)"
}
...
league_tbl |>
  ...
  gt::tab_caption(paste("Classificação: ", params$length_m, "m mais rápidos em", date_short)) |>
  gt::tab_options(
    table.font.size = gt::px(12),
    data_row.padding = gt::px(2),
    column_labels.padding = gt::px(4)
  ...
  tempo~gt::px(60), avg_speed_kmh~gt::px(90)) |>
    gt::tab_footnote("Os componentes (vento/corrente) são calculados como Velocidade * cos(ângulo canoa vs corrente/vento). Medem, portanto, a intensidade do vento ou corrente na direção seguida pela canoa.")
```
