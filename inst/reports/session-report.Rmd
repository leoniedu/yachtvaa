...
Treino de **`r date_long`** -- GPS via Treinus, boia SIMCOSTA 515. Trechos mais rápidos de 500 m vs. vento e corrente.
...
```{r fastest-map, fig.height = 5, fig.cap = "Trechos mais rápidos de 500 m em linha reta. Setas indicam a direção do percurso."}
arrow_df <- fast500 |>
  as_tibble()
...
**Topo = a favor**, **base = contrário**. Raio = intensidade; cor = velocidade do atleta.
...
league_fmt |>
  mutate(
    ...
    wind_class            = wind_class_pt[wind_class],
    wind_component_kmh    = round(wind_component_kmh, 1),
    current_class         = current_class_pt[current_class],
    current_component_kmh = round(current_component_kmh, 1)
  ) |>
  select(
    rank, fullname_athlete, tempo, avg_speed_kmh,
  ...
  gt::cols_label(
    ...
    fullname_athlete = "Atleta",
    tempo = "Tempo",
    avg_speed_kmh = "Vel. (km/h)",
    wind_class = "Vento",
    wind_component_kmh = "Vento (km/h)",
    current_class = "Corrente",
    current_component_kmh = "Corrente (km/h)"
  ) |>
